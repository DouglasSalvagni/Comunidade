...
async function run() {
    try {
        ...
        await dataSource.query(`UPDATE legal_documents SET is_active = false WHERE type = 'PRIVACY_POLICY' AND is_active = true`);
...
Você pode alterar suas preferências de cookies no rodapé do site em "Gerenciar Cookies".`;
        ...
        await dataSource.query(`INSERT INTO legal_documents (id, type, content, is_active, created_at) VALUES ($1, $2, $3, $4, NOW())`, [id, 'PRIVACY_POLICY', privacyContent, true]);
...
run();
//# sourceMappingURL=update-legal-docs.js.map