# Termos de Uso — Ninaro

Estes Termos de Uso ("Termos") regem o acesso e uso da plataforma **Ninaro**, incluindo nosso site, aplicativo móvel e serviços de streaming de áudio (coletivamente, "Serviços").

Ao criar uma conta ou utilizar os Serviços, você concorda em cumprir estes Termos. Se você não concordar, não poderá utilizar o Ninaro.

---

## 1. Definições

- **Serviço:** A plataforma Ninaro de streaming de músicas e audiobooks infantis.
- **Conta:** O registro de usuário criado por um responsável legal (adulto).
- **Perfil Infantil:** Sub-perfis criados dentro de uma Conta para uso por crianças.
- **Conteúdo:** Músicas, áudios, imagens, textos e outros materiais disponibilizados no Serviço.
- **Assinatura:** O plano pago que concede acesso total ou parcial ao Conteúdo.

---

## 2. Aceite e Capacidade Civil

- **Idade Mínima:** Para criar uma Conta e aceitar estes Termos, você deve ter **pelo menos 18 anos** e plena capacidade civil.
- **Responsabilidade:** Ao criar Perfis Infantis, você declara ser o pai, mãe ou responsável legal da criança e autoriza o uso do Serviço por ela sob sua supervisão.
- **Vínculo:** Estes Termos constituem um contrato vinculante entre você e o Ninaro.

---

## 3. Sua Conta e Segurança

- **Credenciais:** Você é responsável por manter a confidencialidade de sua senha. Não compartilhe suas credenciais com terceiros.
- **Veracidade:** Você concorda em fornecer informações verdadeiras, exatas e completas no cadastro (como nome e e-mail).
- **Uso Indevido:** Qualquer atividade realizada através da sua conta é de sua responsabilidade. Notifique-nos imediatamente em `privacidade@empresa.com` se suspeitar de uso não autorizado.
- **Login Social:** Se utilizar login via terceiros (ex: Google), você também está sujeito aos termos desse provedor.

---

## 4. Regras de Uso e Conduta

Você concorda em **NÃO**:
1.  Compartilhar sua conta com pessoas fora do seu núcleo familiar (a licença é para uso pessoal e familiar).
2.  Copiar, redistribuir, transmitir, vender ou explorar comercialmente qualquer Conteúdo do Ninaro.
3.  Tentar contornar medidas de proteção tecnológica (DRM), bloqueios geográficos ou sistemas de autenticação.
4.  Realizar engenharia reversa, descompilar ou tentar obter o código-fonte do aplicativo.
5.  Utilizar *bots*, *scrapers* ou ferramentas automatizadas para coletar dados ou acessar o Serviço.
6.  Sobrecarregar ou tentar derrubar nossos servidores (ataques DDoS).

A violação destas regras pode resultar na suspensão ou banimento imediato da sua conta, sem prejuízo das medidas legais cabíveis.

---

## 5. Assinatura, Pagamentos e Cancelamento

- **Planos:** O acesso ao catálogo completo pode exigir uma Assinatura paga. Detalhes sobre preço, periodicidade e recursos estão disponíveis na página de assinatura.
- **Cobrança:** Os pagamentos são processados por gateways parceiros (ex: Asaas). Ao assinar, você autoriza a cobrança recorrente no método de pagamento escolhido.
- **Renovação Automática:** As assinaturas são renovadas automaticamente ao final de cada período, a menos que você cancele antes da data de renovação.
- **Cancelamento:** Você pode cancelar a renovação automática a qualquer momento nas configurações da conta. O acesso permanecerá ativo até o fim do período já pago.
- **Reembolso:** O direito de arrependimento será respeitado conforme o Código de Defesa do Consumidor (7 dias para compras online), a contar da data da assinatura inicial. Solicitações devem ser enviadas ao nosso suporte.
- **Reajustes:** Reservamo-nos o direito de alterar os preços das assinaturas, mediante aviso prévio razoável.

---

## 6. Conteúdo e Propriedade Intelectual

- **Licença Limitada:** Concedemos a você uma licença limitada, não exclusiva, intransferível e revogável para acessar e ouvir o Conteúdo apenas para **uso pessoal e não comercial**.
- **Propriedade:** Todo o Conteúdo, marcas, logotipos, interface visual e software são propriedade do Ninaro ou de seus licenciadores e são protegidos por leis de direitos autorais e propriedade intelectual.
- **Alterações no Catálogo:** O catálogo de músicas e histórias é dinâmico. Podemos adicionar, remover ou substituir conteúdos a qualquer momento, sem aviso prévio, dependendo de acordos de licenciamento.

---

## 7. Recursos Offline (Downloads)

- Se o Serviço permitir o download de conteúdo para ouvir offline:
  - O conteúdo baixado só pode ser reproduzido dentro do aplicativo Ninaro.
  - O acesso offline é temporário e vinculado à validade da sua assinatura.
  - Se você cancelar a assinatura, sair da conta (logout) ou desinstalar o app, o conteúdo baixado será removido ou perderá o acesso.

---

## 8. Disponibilidade e Suporte

- **Nível de Serviço:** Esforçamo-nos para manter o Serviço disponível 24/7, mas não garantimos que será ininterrupto ou livre de erros. Manutenções programadas ou falhas técnicas podem ocorrer.
- **Requisitos:** A qualidade do streaming depende da sua conexão com a internet e do dispositivo utilizado.
- **Suporte:** Oferecemos suporte técnico por e-mail ou canais indicados na plataforma, respondendo dentro de prazos comerciais razoáveis.

---

## 9. Limitação de Responsabilidade

Na extensão máxima permitida pela lei aplicável:
- O Ninaro é fornecido "como está" (*as is*), sem garantias implícitas de adequação a um fim específico.
- Não nos responsabilizamos por danos indiretos, incidentais ou consequentes, nem por falhas causadas por terceiros (ex: indisponibilidade da sua operadora de internet, falhas no gateway de pagamento).

---

## 10. Modificações nos Termos

Podemos alterar estes Termos a qualquer momento.
- Se houver alterações significativas, notificaremos você (por e-mail ou aviso no app).
- A continuidade do uso do Serviço após as alterações constitui aceitação dos novos Termos.

---

## 11. Disposições Gerais e Foro

- **Lei Aplicável:** Estes Termos são regidos pelas leis da República Federativa do Brasil.
- **Foro:** Fica eleito o foro da comarca de Porto Alegre - RS, para dirimir quaisquer dúvidas ou litígios oriundos destes Termos, com renúncia a qualquer outro, por mais privilegiado que seja.

---

**Contato:**
Dúvidas sobre os Termos? Fale conosco em: `suporte@empresa.com` ou `privacidade@empresa.com`.

**Dados da Empresa:**
Wiser Wizer Tecnologia Aplicada LTDA
41.326.511/0001-85
Porto Alegre - RS
